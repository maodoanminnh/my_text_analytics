# General imports
# FIX 1: collections.Mapping is deprecated in Python 3.10+
...
class TextAnalytics:
    ...
    def cluster(x, y="Missing", k=None, ari=False):
        ...
        # FIX 2: algorithm="full" is deprecated in scikit-learn
        cluster = KMeans(n_clusters=k,
                         init="k-means++",
                         n_init=10,
                         max_iter=10000,
                         tol=0.0001,
                         # copy_x=False, # Deprecated and removed
                         algorithm="lloyd")
        ...
        try:
            ...
            if y == "Missing":
                y = [0 for _ in range(x.shape[0] if not isspmatrix(x) else x.get_shape()[0])]
    ...
    def linguistic_distance(x, y, sample=1, n=1, metric="euclidean"):
        ...
        if isspmatrix(x_sample):
            x_sample = x_sample.toarray() # Use .toarray() instead of .todense()
        ...
        for i in range(x.shape[0]):
            if i != sample:
                ...
                if isspmatrix(x_test):
                    x_test = x_test.toarray() # Use .toarray() instead of .todense()

                # Calculate distance
                if metric == "euclidean":
                    distance = euclidean(x_sample.flatten(), x_test.flatten()) # Ensure 1D array
                else:
                    distance = cosine(x_sample.flatten(), x_test.flatten()) # Ensure 1D array
    ...
    def train_word2vec(self, df, min_count=None, workers=1, language='en', nlp=None):
        ...
        # vocab = embeddings.wv.key_to_index # This is correct for modern Gensim
    ...
    def train_lda(self, df, n_topics, min_count=2, labels=None, tag=False):
        ...
        lda = LdaModel(common_corpus, 
                       num_topics=n_topics,
                       id2word=common_dictionary, # Added for modern Gensim
                       distributed=False,
                       passes=10,
                       iterations=10,
                       )
    ...
    def use_lda(self, df, y, cleaned=False):
        ...
        for i in range(len(corpus)):
            # vector = self.lda[corpus[i]] # Old way
            vector = self.lda.get_document_topics(corpus[i], minimum_probability=0.0) # New way
            label = y[i]
            main = 0.0
            main_index = 0
    ...
    def print_vector(vector, vectorizer):
        ...
        vector = pd.DataFrame(vector.toarray()[0], columns=columns).T # Use .toarray()
        return vector
    ...
    def unmasking(self, df, labels, features, classifier="lr"):
        ...
        train_x = pd.DataFrame(train_x.toarray()) # Use .toarray()
        test_x = pd.DataFrame(test_x.toarray()) # Use .toarray()
        ...
        for i in range(0, 100):
            ...
            for index, row in weights.iterrows():
                # Correct way to get max index for sparse dataframes
                max_val = row.abs().max()
                max_index = row[row.abs() == max_val].index[0]
                
                # Original code might fail if columns are not standard integer index
                # This is a safer way
                col_to_drop = train_x.columns[max_index]
